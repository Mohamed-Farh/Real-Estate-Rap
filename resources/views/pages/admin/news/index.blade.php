...
@section('title')
    {{ trans('front_trans.company_news') }}
@stop
@endsection
@section('page-header')
<!-- breadcrumb -->
    ...
    <div class="card card-statistics h-100">
        <div class="card-body">
            ...
            <button type="button" class="button x-small">
                <a href="{{ route('news.create') }}">{{ trans('front_trans.add_news') }}</a>
            </button>
            <br><br>
                ...
                <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>{{ trans('front_trans.news') }}</th>
                            <th>{{ trans('front_trans.new_body') }}</th>
                        ...
                        @foreach ($news as $new)
                            <tr>
                                <?php $i++; ?>
                                    ...
                                    @if (App::getLocale() == 'en')
                                        @if ($new->body_en !='')
                                            <th>{{ \Str::limit($new->body_en,25)}}</th>
                                        @else
                                            <th>{{ \Str::limit($new->body_ar,25)}}</th>
                                        @endif
                                    @else
                                        <th>{{ \Str::limit($new->body_ar,25)}}</th>
                                    @endif
                                        ...
                                        <div class="modal-body">
                                                <div class="row" style="border: ridge;">
                                                    <div class="col-12 small_space">
                                                        <h3>{{ trans('front_trans.news_ar') }}</h3>
                                                        <h6>{{ $new->head_ar }}</h6><br>
                                                        <h6>{{ $new->body_ar }}</h6>
                                                    </div>
                                                </div>
                                                <div class="row" style="border: ridge;">
                                                    <div class="col-12 small_space">
                                                        <h3>{{ trans('front_trans.news_en') }}</h3>
                                                        <h6>{{ $new->head_en }}</h6><br>
                                                        <h6>{{ $new->body_en }}</h6>
                                                    </div>
                                                </div>
                                                <br><br>
